  ...
  return (
    <div
      className={clsx(
        ...
        hidelogin ? "-translate-y-[60vh]" : "translate-y-3"
      )}
    >
      <div className="top w-full h-[5rem] flex flex-row justify-between items-start py-[1rem]">
        <div className="title text-quicksand text-black">
          <h1 className="font-[600] text-[20px]">Dropdown Mensxascau</h1>
        </div>
        <div className={clsx("close cursor-pointer hover:text-[#c90606]",
            dark ? 'text-white' : 'text-black'
        )} onClick={hideloginmenu}>
          X
        </div>
      </div>
      <div className="bot h-full">dvsvkesvkv ek</div>
    </div>
  );
}
